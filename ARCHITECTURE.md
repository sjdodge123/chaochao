...
| `engine.js` | ~730 | Physics & collision. Per-tick updates hazards/projectiles/players; `QuadTree` broadphase against active map tiles. Helpers used from game.js/entities: `preventEscape`, `checkCollideCells`, `punchPlayer`, `puckPlayer`, `explosion`, `bounceOffBoundry`, `bounceOffSegment` (rotated-segment bounce for the Laser Gate), `barrierCrossing` (first point a segment crosses a map wall/fence — blocks the Sentry Turret's line of sight + its bolt), … |
...
| `projectiles.js` | ~200 | `Projectile` (extends `Circle`) + `CloudProj`, `SnowFlakeProj`, `BombProj`, `TurretShot` (the Sentry Turret's shot — rides the projList wire with its OWN `type: "turretShot"` so the client gives it a unique red-bolt visual + fire/impact SFX, NOT the snowflake sprite/sounds; resolves as a recoverable **shove** with NO terrain mutation; detonates on the first live racer it touches or on its fuse, setting `shove` for `gameBoard.shoveShot`), `Puck`. |
| `hazards.js` | ~360 | `HazardRail`, `Hazard`, `Bumper`, `BumperWall` (rotated-rect slingshot line), `Rotor` (bumper orbiting a pivot — `advance(dt)` motion hook, `streamAngle` wire), `Geyser` (timed dormant→charge→erupt vent — phase timer in `update(dt)`, `netState` wire), `Mine` (proximity-tripped one-shot blast — `netState` wire), `VortexWell` (circular **force zone** — a continuous inward pull rather than a one-shot Punch, with a **calm-eye** profile `force·4r(1−r)`: zero at the dead centre and rim, peak in the mid-ring, so the centre stays escapable; `isVortex`; **per-instance authored radius** — drag-to-resize in the editor, clamped to config `[minRadius, radius]` by `vortexWellRadius()`, shipped in `newHazards` slot `[8]`), `LaserGate` (a laser barrier on a timed open→warn→solid cycle — the first **timed passability gate**; a thin rotated `Rect` like the bumper wall, phase timer in `update(dt)` + `netState` wire, `isLaserGate`; SOLID = a non-lethal **bounce** via `engine.bounceOffSegment` — the rotated-segment cousin of `bounceOffBoundry`, reverting the crossing + reflecting the normal velocity — chosen over a lava-burn so the gate is recoverable and a hidden-timing death can't read unfair), `Crusher` (a Thwomp slab sliding a corridor — **railed** like the moving bumper but owns its motion via `advance(dt)` so the broadside slab keeps a fixed orientation; collided with the bumper-wall rotated-rect machinery on a **centered** `getVertices`; mid-rail contact is a map-owned shove Punch, the outer-`pinchFraction` slam is a lethal `killSelf("crush")`; `isCrusher`, reuses the AI's moving-bumper rail-crossing logic + `railCells` penalty for free), `Turret` (a stationary sentry that tracks the nearest racer in its mount arc and fires an aimed shot every cooldown — the **first projectile-emitting** map element; `streamAngle` ships the live barrel facing + `netState` the idle→charging→firing phase; the aim + state machine live in `serve(playerList, dt, live, map)` called from `gameBoard.updateHazards` since the stationary `update()` hook gets no player list; `acquireTarget` picks the nearest of {racers, antlions} in arc/range, skips a target shielded behind a wall/fence (`engine.barrierCrossing` line-of-sight), and an in-flight bolt is absorbed at a barrier (`gameBoard.updateProjectiles`), so a barrier fully protects whoever's behind it; an antlion hit is knocked back via its `impVX/impVY` impulse channel (`gameBoard.shoveAntlions`, the thumper-slam mechanism — the kart explosion force can't move a chase-steered antlion); it raises `fireRequest` and `gameBoard.fireTurret` spawns a `TurretShot` onto the projList wire with its own `turretShot` type → a unique red-bolt visual + fire/impact SFX, NOT the iceCannon's snowflake; the shot **shoves** rather than freezing terrain (an auto-firing turret reusing iceCannon's freeze would runaway-ice the arena), via `gameBoard.shoveShot`/`applyExplosionForce`; `isTurret` cone branch in the AI), `MagpieDrone` (a thieving drone — the **first hazard to touch the ability ECONOMY**; **railed** like the moving bumper, so motion + AI are free; on body contact it STEALS the racer's held `ability` and carries it as `loot`, shipped on the `netState` wire so the client draws the carried-ability icon, or drains a stamina chunk from an empty-handed racer; a real player PUNCH while carrying makes it DROP the loot — `gameBoard.dropAbilityNear` re-paints the nearest drivable cell as that ability TILE so it's re-grabbable via the normal engine ability-pickup, no parallel pickup system. `handleHit` only RECORDS a steal/drop request (idempotent under the 2×/pair fire); `gameBoard.serveMagpieDrone` does the inventory mutation + the `magpieSteal`/`magpieDrop` broadcasts in the once-per-tick `updateHazards` pass, gated on racing/collapsing. `isMagpie`: the only new AI is a `hazardRepulsion` term making an *ability-carrying* bot give a hungry (empty) drone a wide berth — the rail-crossing/`railCells` handling + the `mapClassifier` lane penalty are reused for free since it's railed), `WarpPad` (one half of a **paired teleporter**, registered as a **BOON** in `boons.js` (`config.boons.warpPad`, id 958) though the class + cross-cutting machinery live here; driving onto pad A relocates a racer to its partner pad B preserving velocity. Pads are authored as a pair via a `pair` integer on each map entry, linked at load by `linkWarpPads`; STATIC, the pair id rides the `netState` wire so the client colour-matches halves. **Not instant — a 2-stage transit:** `gameBoard.updateWarpPads` (one pass/tick, NOT `handleHit` which fires twice/pair) freezes the racer in place + invulnerable for a **distance-based `transitMs`** (`linkWarpPads` sets `pad.transitMs = clamp(dist/warpSpeed·1000, min, max)` from the pair separation, so a longer hop reads as a longer journey; engine `updatePlayers`, the collision pass, `aiController`, the bonus-orb/locked-door objective passes, and solo-collapse-origin all skip a `warping` racer) while the local client **pulls the camera back and pans it to the exit** (`draw.js warpCam` + `WARP_ZOOM_OUT`, following the pan DIRECTLY/`a=1` so it isn't lagged by the usual exponential filter), then EMERGES it at the partner with velocity restored; `warpStart`/`warpEnd` events drive the client SFX (`audio.js playWarpEnter`/`playWarpExit`), the camera, the kart-hide, and a **combat-log "🌀 warped" entry**. A death mid-transit drops it (no ghost emerge); `Player.reset`/`killPlayer` clear it. A per-player `warpArmed` latch stops ping-pong (re-arms only off all pads). `helpful` (a boon) makes the AI SKIP it as an obstacle — instead `cellGraph.getWarpLinks` adds a graph edge between the two linked cells **weighted by the per-pair transit cost** (`cellGraph.warpTransitMs`, matching the runtime, × cruise) so bots route THROUGH the pair only when it shortens the trip by more than the freeze, and `estimatePathTime` adds the per-pair transit seconds to the hop. The recap (auto, via the live `hazardDrawers`) and the next-map thumbnail (`drawThumbnailBoonGlyph`, per-pair colour + a portal glyph) both show it. **Fairness guard:** the optimal-path par/fairness can't see a *trap* — a pad whose goal-side mouth sits on the racing line flings a naive racer backward, while the optimal route takes the pair from its start side — so `mapClassifier.warpTrapSeverity` (using a `noWarp` `findPathToNearestGoal` to measure each pad's drive-distance to goal + whether the goal-side mouth sits within `warpTrapRadius` of a no-warp racing line) scores the worst backward setback into a `warptrap` deduction (and a hard-fail for a catastrophic one), shown in the editor's balance overlay), plus the **hazard-kind registry** (`HAZARD_KINDS`, `hazardKindById`, `registerHazardKind`) — single source of truth for map-authorable hazard kinds. **Adding a hazard kind touches:** a `config.json` `hazards` entry, one `registerHazardKind` call here (with `railed`/`directional` + `build`), a drawer in `client/scripts/draw.js` (`buildHazardDrawers`), and an editor entry in `client/scripts/create.js` (`EDITOR_HAZARD_KINDS`, with optional `paint`/`swatchPaint`/`segmentSelect` hooks for non-disc kinds). **Per-tick motion/state:** moveable kinds either ride a `rail` or define `advance(dt)` (engine.updateHazards); stationary stateful kinds run a phase timer in `update(dt)` (gameBoard.updateHazards passes dt) and ship a `netState` int. **Force-zone kinds** (`forceZone: true`) apply a continuous force via `applyForce(player)`, called ONCE per tick per player by `gameBoard.updateHazards` (NOT in `handleHit` — `engine.narrowBase` fires handleHit up to twice per overlapping pair, which would double the force; handleHit is a no-op for these), only while racing/collapsing, self-filtering by distance and skipping protected/star-power karts like `applyExplosionForce`. The AI gets an `isVortex` (route around the core), `isLaserGate` (steer off the beam only while it's closing — `railCells`/timeable), and `isTurret` (price the firing cone at the mild `railCells`/timeable penalty + a lateral `hazardRepulsion` term to keep bots out of the line of fire) branch in `aiController` and `mapClassifier`; the railed `Crusher` reuses the existing moving-bumper rail-crossing logic with no new branch. `gameBoard.generateHazards`, `utils.validateMap` (`directional` ⇒ finite angle), the lightning speed-up, the editor palette/swatch/thumbnail/shortcut, and the wire payloads are all registry-driven and need no changes. |
| `boons.js` | ~80 | `Boon` (extends `Hazard`) + `DashArrows`, `RechargeSpring`, `Slipstream`, `GuardHalo`, `SecondWindTotem`, and the **Warp Pad** `registerBoonKind` (the `WarpPad` class itself lives in `hazards.js` with its cross-cutting machinery — `linkWarpPads`, `gameBoard.updateWarpPads`, the `cellGraph` shortcut — but registers as a boon here so `config.boons.warpPad` supplies its id and the editor files it under Boons). The **helpful** counterpart to hazards. Boons register into the SAME kind registry via `registerBoonKind` (which stamps `helpful: true` and reads ids from `config.boons`, 950+), so they live in the same runtime `hazardList` and reuse the hazard wire, drawer, validator, and editor pipeline unchanged. `helpful` is the only divergence — it gates the lightning speed-up, the AI's `hazardRepulsion`/cell-penalty (bots don't dodge a boost), and `mapClassifier`'s difficulty count. Loaded for its side-effect registrations by `hazards.js` (after that module's exports). **Adding a boon kind touches:** a `config.json` `boons` entry, one `registerBoonKind` call here (with `directional` + `build`), a drawer in `client/scripts/draw.js` (`buildHazardDrawers`), and an editor entry in `client/scripts/create.js` (`EDITOR_HAZARD_KINDS` with `group: "boon"` + a `paint`/`swatchPaint`). The map array (`hazards[]`), wire, generateHazards, validateMap, AI gate, and classifier skip are all registry/flag-driven and need no changes. |
