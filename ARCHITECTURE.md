...
| `hazards.js` | ~210 | `HazardRail`, `Hazard`, `Bumper`, `BumperWall` (rotated-rect slingshot line; overrides `getVertices`/`getExtents`, punches from the nearest centerline point), plus the **hazard-kind registry** (`HAZARD_KINDS`, `hazardKindById`, `registerHazardKind`) — single source of truth for map-authorable hazard kinds. **Adding a hazard kind touches:** a `config.json` `hazards` entry, one `registerHazardKind` call here (with `railed`/`directional` + `build`), a drawer in `client/scripts/draw.js` (`buildHazardDrawers`), and an editor entry in `client/scripts/create.js` (`EDITOR_HAZARD_KINDS`, with optional `paint`/`swatchPaint`/`segmentSelect` hooks for non-disc kinds). `gameBoard.generateHazards`, `utils.validateMap` (`directional` ⇒ finite angle), the lightning speed-up, the editor palette/swatch/thumbnail/shortcut, and the wire payloads are all registry-driven and need no changes. |
