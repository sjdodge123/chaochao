...
### 1. Teach AI to punch-swim — ✅ DONE

> **Shipped.** Bots now punch-swim: `aiController.decideAttack` strokes (a bare-handed tap)
> toward the steer vector `targetDirX/Y` when `bot.onWater`, and `player.throwChargedPunch`'s
> swim branch falls back to that vector for AI (`getMoveDir()` is null without move keys), so
> the existing `swimImpulse` just works — human feel byte-for-byte unchanged (the fallback is
> `isAI`-guarded). `cellGraph.tileWeight(water)` dropped 13→6.5: the HONEST cost, since
> swimming is stamina-paced (~11-13 px/s sustained vs grass ~78, measured headless ≈6× grass-
> time) — so bots route through water only when it's genuinely the faster/only line and take a
> dry lane otherwise. Verified by `.github/scripts/ai-swim-test.js` (dry-lane preference at the
> pathfinding level + a bot completing a water-only crossing all the way to gameOver, so a
> pinned water playlist can't stall). Original prompt preserved below.
