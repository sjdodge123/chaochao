...
```
Continue the Chao Chao map-hazards initiative. Read docs/hazards/hazard-sprint-plan.md first — it's the living design doc (framework recipe §2, conventions §3, roadmap §4, per-batch specs §5, chaining workflow §6, status log §7). Batch 2 (Vortex Well, id 908) shipped via PR #310 → v0.40.0; the Gust Fan was cut. The next free hazard id is 909.

Do Batch 3: Blink Fence + Crusher — the two state-machine / sliding-wall hazards (§5). Use id 909 for the first kind, 910 for the second. New bits vs prior batches:
...
Create the worktree off origin/main: git worktree add -b worktree-hazard-batch3 .claude/worktrees/hazard-batch3 origin/main. Follow the add-a-kind recipe (§2): config + registry + draw + editor + headless test wired into pr-validation.yml, plus CHANGELOG/Codex(learn.js)/ARCHITECTURE. LOCKSTEP checks Batch 2 learned the hard way: if you add ANY newHazards wire field, update .github/scripts/unit-tests.js (it pins the creation-row field count) AND the client applyHazards decoder together; if you touch antlion/zombie steering or add a brutal-round interaction, cover it in antlion-test.js. Then run the FULL headless battery (unit-tests, smoke, classifier, validate-content, codex-coverage, mode-smoke, antlion-test, the AI tests since you'll touch aiController, plus your new test) + /codex:review (high), fix findings with regression coverage, and host a dev server (localhost + LAN IP) for playtest. To force a brutal round in editor-preview playtest, set config.brutalTypesForce=[<id>] at runtime (there is no committed seam).

Honor the gates: NO push/PR without explicit operator go-ahead each time. Rebase onto origin/main and re-verify the FULL battery before any PR (main moves fast — Batch 2 hit a rebase conflict AND a stale unit-test only the full battery caught). Babysit CI after opening (inline bot comments via gh api repos/sjdodge123/chaochao/pulls/<n>/comments + /reviews; start-edges-test, the antlion-test chase session, and client-perf are known flakes/noise — re-run, don't chase). Use release:minor for a new placeable. Update §4/§5 + the Status Log (§7) in the design doc in the same PR, and write the Batch 4 (Sentry Turret) handover prompt at the end.

Reference Batch 2 (Vortex Well force zone) as the closest precedent for the registry/editor/AI/test wiring; reference movingBumper (rail) + bumperWall (rotated-rect collision) for the Crusher, and the geyser (netState phase timer) for the Blink Fence.
```
