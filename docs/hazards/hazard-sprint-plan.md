...
- **Blur effect:** the interior renders as a hazy violet swirl (`draw.js drawVortexWell`). The blur is baked ONCE into an offscreen sprite via `ctx.filter = "blur()"` (the `getBlackoutHoleSprite` pattern) and rotate-blitted each frame — a per-frame canvas filter is a mobile GPU killer (see the cosmetic-perf-collapse history), so the filter never runs in the live loop. It's a stylized haze, NOT a true frosted-glass blur of the terrain/karts behind it (that would need a per-frame framebuffer capture+blur — deferred as too costly).
