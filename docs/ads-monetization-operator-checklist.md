...
- GameMonetize adapter (headless, 18/18): `SDK_READY`→ready, `SDK_GAME_START`→
  complete (incl. no-fill), `SDK_ERROR`/loader-404→no-op, 8 s timeout→`ad_error`
  reason=timeout, missing `window.sdk`→`ad_error` reason=error. ✅
- Fail-open: SDK timeout (8 s) / throw / 404 all fire `onClose` and log
  `ad_error`; gameplay proceeds. ✅
