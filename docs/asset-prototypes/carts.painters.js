/*
 * chaochao — FROZEN cart-body painters  (carts asset-design session, 2026-05-30)
 * Source of truth: docs/asset-prototypes/carts.html (operator-approved + frozen).
 * Extracted verbatim from the tested prototype so it cannot drift. Reference module —
 * the main cosmetics session ports these into client/scripts/draw.js (painters) and
 * client/scripts/skinRegistry.js (see FROZEN_CART_REGISTRY at the bottom).
 *
 * PAINTER CONTRACT (matches draw.js drawCartSkin):
 *   function drawXxxSkin(ctx, anim, paint, heading) { ... }
 *   - Normalized space: radius == 1, drawn in a unit circle, forward = +X.
 *     The integrator does translate / rotate / scale.
 *   - anim   = rolling animation time in seconds.
 *   - paint  = the player's CSS colour string; the body MUST tint to it (use cartSkinShade/A).
 *   - heading= travel angle in RADIANS (0 = +X). Only the heading-tracking carts read it.
 *
 * HELPERS:
 *   - cartSkinShade / cartSkinRGB / cartRoundRectPath / drawSkinCar / cartFillBody ALREADY
 *     exist in draw.js — the copies here are just so this file is self-contained.
 *   - cartSkinShadeA (rgba shade) and cartPolyPath (regular-polygon path) are NEW: add them to draw.js.
 *   - drawTruckSkin / drawFiretruckSkin / drawDinoSkin are REFERENCE copies (already in draw.js).
 *   - PARAMS was the prototype's live-tuning object; it is {} here, so every painter falls back to
 *     its inlined default (the value after ':' in  PARAMS.x != null ? PARAMS.x : DEFAULT). When
 *     porting, just inline the DEFAULT and delete the PARAMS lookup.
 *
 * INTEGRATION — drawCartSkin needs two small changes to support the "statue" carts:
 *   (1) STATUE carts must NOT be rotated by heading — draw them upright (skip ctx.rotate(heading)).
 *   (2) Pass heading (radians) as the 4th painter arg. Non-statue carts ignore the extra arg, so
 *       this is backward-compatible with the existing painters.
 *   The registry below flags { statue, tracksHeading, stateful } per cart.
 *     - tracksHeading carts (golden_champion lucky-cat eyes, compass needle, googly eyeball pupil)
 *       read heading to point toward travel — they REQUIRE both (1) and (2).
 *     - stateful carts (eight_ball shake->answer, wheel_of_fortune spin->settle) keep module-level
 *       state and use Math.random; fine on the game client.
 *     - ok_hand draws the literal U+1F44C emoji glyph: it does NOT tint to the player colour (one-off).
 *
 * PATTERNS (a SEPARATE equip slot / separate session): only Flames & Nebula were reworked here
 * (now real see-through patterns with no opaque backdrop) and are included for that session.
 * The prototype also proved pattern BLEND modes via ctx.globalCompositeOperation:
 *   Over = source-over (paint on top) | Multiply = darken into body |
 *   Inside = source-atop (mask to the true silhouette) | Cut = destination-out (engrave/subtract).
 * Recommend giving drawPatternOverlay a per-pattern (or global) blend mode.
 *
 * REMOVED / AVOIDED FOR IP OR CLARITY: "Capsule Ball" (Poke-ball trade dress), "Bomb" (read as the
 * explosive ability), "Radar", and a literal Mickey Mouse (Disney TM — shipped a generic top-down
 * Mouse instead). The base "Truck"/crimson cart is also slated for removal from the registry.
 */
var PARAMS = {};
...
// rgba variant for glows (same hue family as paint, with alpha).
...
function drawSkinCar(ctx, anim, decorate) {
  var wheelR = 0.32;
  var wheels = [[0.46, 0.6], [0.46, -0.6], [-0.46, 0.6], [-0.46, -0.6]];
  for (var i = 0; i < wheels.length; i++) {
    ...
    ctx.rotate(anim * 3);
    ctx.strokeStyle = "#666"; ctx.lineWidth = 0.05;
    for (var s = 0; s < 4; s++) {
      var a = (s * Math.PI) / 2;
      ctx.beginPath(); ctx.moveTo(0, 0);
      ctx.lineTo(Math.cos(a) * wheelR * 0.8, Math.sin(a) * wheelR * 0.8); ctx.stroke();
    }
    ctx.restore();
  }
  ctx.save();
  cartRoundRectPath(ctx, -0.74, -0.5, 1.48, 1.0, 0.2); ctx.clip();
  decorate(ctx);
  ctx.restore();
  ctx.fillStyle = "rgba(190,230,255,0.85)";
  cartRoundRectPath(ctx, 0.26, -0.3, 0.34, 0.6, 0.08); ctx.fill();
  ctx.strokeStyle = "rgba(0,0,0,0.55)"; ctx.lineWidth = 0.07;
  cartRoundRectPath(ctx, -0.74, -0.5, 1.48, 1.0, 0.2); ctx.stroke();
}
function cartFillBody(ctx, style) { ctx.fillStyle = style; ctx.fillRect(-0.85, -0.6, 1.7, 1.2); }

// ============================================================================
// REFERENCE painters (existing, copied verbatim from draw.js for style compare).
// ============================================================================
function drawTruckSkin(ctx, anim, paint) {
  drawSkinCar(ctx, anim, function (c2) { cartFillBody(c2, paint || "#c0182b"); });
}
function drawFiretruckSkin(ctx, anim, paint) {
  paint = paint || "#d11f1f";
  var wheelR = 0.42;
  var wheels = [[0.5, 0.72],[0.5, -0.72],[-0.5, 0.72],[-0.5, -0.72]];
  for (var i = 0; i < wheels.length; i++) {
    ctx.save(); ctx.translate(wheels[i][0], wheels[i][1]);
    ctx.beginPath(); ctx.arc(0, 0, wheelR, 0, Math.PI * 2); ctx.fillStyle = "#1a1a1a"; ctx.fill();
    ctx.beginPath(); ctx.arc(0, 0, wheelR * 0.45, 0, Math.PI * 2); ctx.fillStyle = "#cfcfcf"; ctx.fill();
    ctx.rotate(anim * 3); ctx.strokeStyle = "#555"; ctx.lineWidth = 0.06;
    for (var s = 0; s < 4; s++) { var a = (s*Math.PI)/2; ctx.beginPath(); ctx.moveTo(0,0);
      ctx.lineTo(Math.cos(a)*wheelR*0.85, Math.sin(a)*wheelR*0.85); ctx.stroke(); }
    ctx.restore();
  }
  ctx.fillStyle = cartSkinShade(paint, -0.05); ctx.strokeStyle = cartSkinShade(paint, -0.45); ctx.lineWidth = 0.07;
  cartRoundRectPath(ctx, -0.78, -0.52, 1.56, 1.04, 0.18); ctx.fill(); ctx.stroke();
  ctx.fillStyle = "#bfe6ff"; cartRoundRectPath(ctx, 0.28, -0.34, 0.4, 0.68, 0.08); ctx.fill();
  ctx.strokeStyle = "#e8e8e8"; ctx.lineWidth = 0.05;
  ctx.beginPath(); ctx.moveTo(-0.55,-0.18); ctx.lineTo(0.15,-0.18); ctx.moveTo(-0.55,0.18); ctx.lineTo(0.15,0.18); ctx.stroke();
  for (var r=0; r<4; r++){ var lx=-0.5+r*0.18; ctx.beginPath(); ctx.moveTo(lx,-0.18); ctx.lineTo(lx,0.18); ctx.stroke(); }
  ctx.fillStyle = "#ffffff"; ctx.beginPath(); ctx.arc(-0.32,0,0.2,0,Math.PI*2); ctx.fill();
  ctx.fillStyle = cartSkinShade(paint, -0.05); ctx.save(); ctx.font = "0.3px sans-serif";
  ctx.textAlign="center"; ctx.textBaseline="middle"; ctx.translate(-0.32,0); ctx.fillText("5",0,0.02); ctx.restore();
}
function drawDinoSkin(ctx, anim, paint) {
  paint = paint || "#43b047";
  var legSwing = Math.sin(anim * 4) * 0.22;
  ctx.fillStyle = cartSkinShade(paint, -0.35);
  var legs = [[0.28,0.55,legSwing],[-0.3,0.55,-legSwing],[0.28,-0.55,-legSwing],[-0.3,-0.55,legSwing]];
  for (var i=0;i<legs.length;i++){ ctx.beginPath(); ctx.ellipse(legs[i][0]+legs[i][2],legs[i][1],0.16,0.1,0,0,Math.PI*2); ctx.fill(); }
  ctx.fillStyle = cartSkinShade(paint, -0.12);
  ctx.beginPath(); ctx.moveTo(-0.55,-0.18); ctx.lineTo(-1.0,0); ctx.lineTo(-0.55,0.18); ctx.closePath(); ctx.fill();
  ctx.fillStyle = paint; ctx.strokeStyle = cartSkinShade(paint, -0.45); ctx.lineWidth = 0.06;
  ctx.beginPath(); ctx.ellipse(-0.05,0,0.6,0.45,0,0,Math.PI*2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = cartSkinShade(paint, -0.35);
  for (var p=0;p<3;p++){ var px=-0.3+p*0.28; ctx.beginPath(); ctx.moveTo(px-0.1,0); ctx.lineTo(px,-0.22); ctx.lineTo(px+0.1,0); ctx.closePath(); ctx.fill(); }
  ctx.fillStyle = paint; ctx.strokeStyle = cartSkinShade(paint, -0.45);
  ctx.beginPath(); ctx.ellipse(0.62,0,0.3,0.26,0,0,Math.PI*2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = "#ffffff"; ctx.beginPath(); ctx.arc(0.72,-0.12,0.07,0,Math.PI*2); ctx.fill();
  ctx.fillStyle = "#111"; ctx.beginPath(); ctx.arc(0.74,-0.12,0.035,0,Math.PI*2); ctx.fill();
}

// ============================================================================
// NEW CART PAINTERS — tunable via window.PARAMS (prototype only). The ported
// versions will inline the chosen constants.
// ============================================================================
var PARAMS = window.PARAMS = {
  hb_len: 0.86, hb_glow: 0.62,
  sf_sweep: 0.58, sf_glow: 0.70,
  gc_trim: 0.60, gc_sheen: 0.55,
  wl_spike: 0.55, wl_plate: 0.60,
};
...
function drawHoverbikeSkin(ctx, anim, paint) {
  paint = paint || "#5ad0ff";
  var len = (PARAMS.hb_len != null ? PARAMS.hb_len : 0.86);   // nose reach (+X)
  var glow = (PARAMS.hb_glow != null ? PARAMS.hb_glow : 0.62);
...
function drawStarfighterSkin(ctx, anim, paint) {
  paint = paint || "#b08bff";
  var sweep = (PARAMS.sf_sweep != null ? PARAMS.sf_sweep : 0.58); // 0=straight,1=hard swept
  var glow = (PARAMS.sf_glow != null ? PARAMS.sf_glow : 0.70);
...
function drawGoldenChampionSkin(ctx, anim, paint, heading) {
  ...
  var pawWave = (PARAMS.gc_trim != null ? PARAMS.gc_trim : 0.60);   // beckon speed/throw
  var sheen = (PARAMS.gc_sheen != null ? PARAMS.gc_sheen : 0.55);
...
function drawWarlordSkin(ctx, anim, paint) {
  paint = paint || "#8a3b2b";
  var spike = (PARAMS.wl_spike != null ? PARAMS.wl_spike : 0.55);
  var plate = (PARAMS.wl_plate != null ? PARAMS.wl_plate : 0.60);
...
// ============================================================================
// PATTERN overlays (from draw.js). In the game these are a SEPARATE equip slot:
// drawPatternOverlay paints the equipped pattern on top of the cart body, CLIPPED
// to the kart silhouette (a 0.95 disc), tinted to the same player colour. They are
// composited AFTER the cart body. Included here so you can see how a pattern reads
// on each shaped cart. (Stripes/Polka/Checkered/Flames are today's painters;
// Nebula/Executioner/Punching Bag are placeholders the patterns session reworks.)
// ============================================================================
function drawStripesPattern(ctx, anim, paint) {
  paint = paint || "#888";
  ctx.fillStyle = cartSkinShade(paint, -0.5);
  ctx.fillRect(-0.85, -0.17, 1.7, 0.13);
  ctx.fillRect(-0.85, 0.04, 1.7, 0.13);
}
function drawPolkaPattern(ctx, anim, paint) {
  ctx.fillStyle = cartSkinShade(paint || "#888", 0.7);
  var row = 0;
  for (var dy = -0.42; dy <= 0.42; dy += 0.28) {
    var off = (row % 2) ? 0.14 : 0;
    for (var dx = -0.7; dx <= 0.7; dx += 0.28) {
      ctx.beginPath(); ctx.arc(dx + off, dy, 0.075, 0, Math.PI * 2); ctx.fill();
    }
    row++;
  }
}
function drawCheckeredPattern(ctx, anim, paint) {
  ctx.fillStyle = cartSkinShade(paint || "#888", -0.6);
  var n = 8, cw = 1.7 / n, chh = 1.2 / n;
  for (var ix = 0; ix < n; ix++) {
    for (var iy = 0; iy < n; iy++) {
      if ((ix + iy) % 2) ctx.fillRect(-0.85 + ix * cw, -0.6 + iy * chh, cw + 0.01, chh + 0.01);
    }
  }
}
function drawFlamesPattern(ctx, anim, paint) {
  paint = paint || "#888";
  // Flame tongues licking up from the back (-X side), in the player's hue, flickering.
  var base = [-0.58, -0.3, -0.02, 0.26, 0.54];
  for (var i = 0; i < base.length; i++) {
    var bx = base[i];
    var h = 0.95 + 0.18 * Math.sin(anim * 6 + i * 1.3);
    var w = 0.17;
    var topY = 0.6 - h;
    var wob = 0.05 * Math.sin(anim * 5 + i);
    var g = ctx.createLinearGradient(0, 0.6, 0, topY);
    g.addColorStop(0, cartSkinShade(paint, -0.1));
    g.addColorStop(0.5, cartSkinShade(paint, 0.4));
    g.addColorStop(1, cartSkinShade(paint, 0.75));
    ctx.fillStyle = g;
    ctx.beginPath();
    ctx.moveTo(bx - w, 0.62);
    ctx.bezierCurveTo(bx - w, 0.2, bx - 0.08, topY + 0.22, bx + wob, topY);
    ctx.bezierCurveTo(bx + 0.08, topY + 0.22, bx + w, 0.2, bx + w, 0.62);
    ctx.closePath(); ctx.fill();
    // inner brighter lick
    ctx.fillStyle = cartSkinShade(paint, 0.85);
    ctx.beginPath();
    ctx.moveTo(bx - w * 0.5, 0.6);
    ctx.quadraticCurveTo(bx - 0.03, (topY + 0.6) / 2, bx + wob, topY + 0.16);
    ctx.quadraticCurveTo(bx + 0.03, (topY + 0.6) / 2, bx + w * 0.5, 0.6);
    ctx.closePath(); ctx.fill();
  }
}
function drawNebulaPattern(ctx, anim, paint) {
  paint = paint || "#888";
  // Cloud puffs in the player hue — NO opaque backdrop, but high-contrast (bright
  // cores + dark dust lanes) so the nebula still READS on a solid same-hue body.
  // [x, y, rx, ry, lite?]
  var puffs = [
    [-0.32, -0.22, 0.55, 0.66, 1], [0.34, 0.06, 0.6, 0.46, 0], [0.02, 0.34, 0.46, 0.6, 1],
    [-0.42, 0.28, 0.4, 0.34, 0], [0.22, -0.4, 0.42, 0.32, 1]
  ];
  for (var i = 0; i < puffs.length; i++) {
    var px = puffs[i][0], py = puffs[i][1], rx = puffs[i][2], ry = puffs[i][3], lite = puffs[i][4];
    var g = ctx.createRadialGradient(px, py, 0.02, px, py, Math.max(rx, ry));
    if (lite) {
      g.addColorStop(0, cartSkinShadeA(paint, 0.8, 0.85));
      g.addColorStop(0.55, cartSkinShadeA(paint, 0.5, 0.4));
      g.addColorStop(1, cartSkinShadeA(paint, 0.5, 0));
    } else {
      g.addColorStop(0, cartSkinShadeA(paint, -0.6, 0.75));
      g.addColorStop(0.55, cartSkinShadeA(paint, -0.55, 0.35));
      g.addColorStop(1, cartSkinShadeA(paint, -0.55, 0));
    }
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.ellipse(px, py, rx, ry, 0, 0, Math.PI * 2); ctx.fill();
  }
  // Bright nebula core glow.
  var core = ctx.createRadialGradient(-0.1, -0.12, 0.02, -0.1, -0.12, 0.5);
  core.addColorStop(0, "rgba(255,255,255,0.55)");
  core.addColorStop(0.4, cartSkinShadeA(paint, 0.7, 0.3));
  core.addColorStop(1, cartSkinShadeA(paint, 0.7, 0));
  ctx.fillStyle = core;
  ctx.beginPath(); ctx.arc(-0.1, -0.12, 0.5, 0, Math.PI * 2); ctx.fill();
  // Twinkling stars.
  var stars = [[-0.5, -0.3], [0.2, -0.4], [0.55, 0.1], [-0.1, 0.35], [-0.6, 0.2], [0.4, 0.42], [0.0, -0.05], [-0.25, -0.52]];
  for (var s = 0; s < stars.length; s++) {
    var tw = 0.35 + 0.65 * Math.abs(Math.sin(anim * 3 + s * 1.7));
    ctx.fillStyle = "rgba(255,255,255," + tw + ")";
    ctx.beginPath(); ctx.arc(stars[s][0], stars[s][1], 0.02 + 0.012 * tw, 0, Math.PI * 2); ctx.fill();
  }
  // A couple of 4-point sparkle stars.
  var big = [[-0.15, -0.28], [0.42, -0.12]];
  for (var b = 0; b < big.length; b++) {
    var t2 = 0.5 + 0.5 * Math.sin(anim * 4 + b * 2);
    var r = 0.06 + 0.03 * t2;
    ctx.strokeStyle = "rgba(255,255,255," + (0.5 + 0.4 * t2) + ")"; ctx.lineWidth = 0.012;
    ctx.beginPath();
    ctx.moveTo(big[b][0] - r, big[b][1]); ctx.lineTo(big[b][0] + r, big[b][1]);
    ctx.moveTo(big[b][0], big[b][1] - r); ctx.lineTo(big[b][0], big[b][1] + r);
    ctx.stroke();
  }
}
function drawExecutionerPattern(ctx, anim, paint) {
  drawFlamesPattern(ctx, anim, paint);
  ctx.fillStyle = cartSkinShade(paint || "#888", -0.7);
  ctx.beginPath(); ctx.arc(0.1, 0, 0.22, 0, Math.PI * 2); ctx.fill();
}
function drawPunchingBagPattern(ctx, anim, paint) {
  paint = paint || "#888";
  var rings = [0.5, 0.34, 0.18];
  for (var i = 0; i < rings.length; i++) {
    ctx.beginPath(); ctx.arc(0, 0, rings[i], 0, Math.PI * 2);
    ctx.fillStyle = (i % 2) ? cartSkinShade(paint, 0.5) : cartSkinShade(paint, -0.55);
    ctx.fill();
  }
}
// A neutral SAMPLE cart body: the plain glossy player disc (tinted to paint),
// matching the canonical body the patterns session previews against. Toggling the
// "sample body" checkbox swaps every cell to this so a pattern reads the way
// drawPatternOverlay paints it on a default cart.
function drawPlainBody(ctx, anim, paint) {
  paint = paint || "#888";
  ctx.beginPath(); ctx.arc(0, 0, 0.95, 0, Math.PI * 2);
  ctx.fillStyle = paint; ctx.fill();
  ctx.save();
  ctx.beginPath(); ctx.arc(0, 0, 0.95, 0, Math.PI * 2); ctx.clip();
  // Glossy sphere shade: lit cap (upper-left) -> base -> dark rim (AO).
  var body = ctx.createRadialGradient(-0.33, -0.38, 0.1, -0.1, -0.1, 1.2);
  body.addColorStop(0, "rgba(255,255,255,0.6)");
  body.addColorStop(0.3, "rgba(255,255,255,0)");
  body.addColorStop(0.62, "rgba(0,0,0,0)");
  body.addColorStop(1, "rgba(0,0,0,0.55)");
  ctx.fillStyle = body; ctx.fillRect(-1, -1, 2, 2);
  // Specular dot.
  var spec = ctx.createRadialGradient(-0.33, -0.4, 0, -0.33, -0.4, 0.4);
  spec.addColorStop(0, "rgba(255,255,255,0.7)"); spec.addColorStop(1, "rgba(255,255,255,0)");
  ctx.fillStyle = spec; ctx.fillRect(-1, -1, 2, 2);
  ctx.restore();
}

var PATTERNS = [
  { name: "None", fn: null },
  { name: "Stripes", fn: drawStripesPattern },
  { name: "Polka", fn: drawPolkaPattern },
  { name: "Checkered", fn: drawCheckeredPattern },
  { name: "Flames", fn: drawFlamesPattern },
  { name: "Nebula", fn: drawNebulaPattern },
  { name: "Executioner", fn: drawExecutionerPattern },
  { name: "Punching Bag", fn: drawPunchingBagPattern },
];
var activePattern = null;   // fn of the equipped pattern, or null
// How the pattern composites onto the cart body (canvas globalCompositeOperation):
//  Over  = paint opaquely on top (today's behaviour)
//  Multiply = darken the body in the pattern shape (keeps the body's shading)
//  Inside = source-atop: pattern shows ONLY where the cart actually drew pixels
//           (masks to the true silhouette, not a disc)
//  Cut   = destination-out: the pattern ERASES/engraves the body (subtractive)
var BLENDS = [
  { name: "Over", op: "source-over" },
  { name: "Multiply", op: "multiply" },
  { name: "Inside", op: "source-atop" },
  { name: "Cut", op: "destination-out" },
];
var patBlend = "source-over";


// ============================================================================
// FROZEN_CART_REGISTRY — port into client/scripts/skinRegistry.js.
//   statue        : true  -> drawCartSkin must NOT heading-rotate it; draw upright.
//   tracksHeading : true  -> pass heading (radians) so the painter can point at travel.
//   stateful      : true  -> painter keeps module-level state (Math.random driven).
//   tintsToColor  : false -> renders in a fixed palette (only ok_hand, the 👌 emoji).
// The first four (hoverbike, starfighter, golden_champion, warlord) REPLACE the placeholder
// cart stubs already wired in draw.js/skinRegistry; the rest are NEW — assign ladder/achievement
// or shop slots as desired. 43 carts total.
// ============================================================================
var FROZEN_CART_REGISTRY = [
  { id: 'hoverbike', display: 'Hoverbike', painter: drawHoverbikeSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#5ad0ff', tintsToColor: true },
  { id: 'starfighter', display: 'Starfighter', painter: drawStarfighterSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#b08bff', tintsToColor: true },
  { id: 'golden_champion', display: 'Golden Champion (lucky cat)', painter: drawGoldenChampionSkin, statue: true, tracksHeading: true, stateful: false, defaultColor: '#c0182b', tintsToColor: true },
  { id: 'warlord', display: 'Warlord', painter: drawWarlordSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#8a3b2b', tintsToColor: true },
  { id: 'pizza', display: 'Pizza Pie', painter: drawPizzaSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#e07b39', tintsToColor: true },
  { id: 'earth', display: 'Planet Earth', painter: drawEarthSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#4a78e0', tintsToColor: true },
  { id: 'smiley', display: 'Smiley', painter: drawSmileySkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#f5d142', tintsToColor: true },
  { id: 'eight_ball', display: 'Magic 8-Ball', painter: drawEightBallSkin, statue: true, tracksHeading: false, stateful: true, defaultColor: '#3f6fe0', tintsToColor: true },
  { id: 'saw_blade', display: 'Saw Blade', painter: drawSawBladeSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#9aa3ad', tintsToColor: true },
  { id: 'donut', display: 'Donut', painter: drawDonutSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#f06ea9', tintsToColor: true },
  { id: 'vinyl', display: 'Vinyl Record', painter: drawVinylSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#d23b6a', tintsToColor: true },
  { id: 'compass', display: 'Compass', painter: drawCompassSkin, statue: true, tracksHeading: true, stateful: false, defaultColor: '#c0392b', tintsToColor: true },
  { id: 'wheel_of_fortune', display: 'Wheel of Fortune', painter: drawWheelSkin, statue: true, tracksHeading: false, stateful: true, defaultColor: '#c0392b', tintsToColor: true },
  { id: 'clock', display: 'Clock', painter: drawClockSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#3f6fe0', tintsToColor: true },
  { id: 'eyeball', display: 'Googly Eyeball', painter: drawEyeballSkin, statue: true, tracksHeading: true, stateful: false, defaultColor: '#4a78e0', tintsToColor: true },
  { id: 'disco_ball', display: 'Disco Ball', painter: drawDiscoSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#9b5de5', tintsToColor: true },
  { id: 'hypno', display: 'Hypno-Spiral', painter: drawHypnoSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#d0342c', tintsToColor: true },
  { id: 'cookie', display: 'Cookie', painter: drawCookieSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#e8a13c', tintsToColor: true },
  { id: 'beach_ball', display: 'Beach Ball', painter: drawBeachballSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#e8453c', tintsToColor: true },
  { id: 'sun', display: 'Sun', painter: drawSunSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#f5b50a', tintsToColor: true },
  { id: 'shuriken', display: 'Shuriken', painter: drawShurikenSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#9aa3ad', tintsToColor: true },
  { id: 'dartboard', display: 'Dartboard', painter: drawDartboardSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#3fb6c8', tintsToColor: true },
  { id: 'soccer_ball', display: 'Soccer Ball', painter: drawSoccerSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#2b2f3a', tintsToColor: true },
  { id: 'basketball', display: 'Basketball', painter: drawBasketballSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#e2802b', tintsToColor: true },
  { id: 'yin_yang', display: 'Yin-Yang', painter: drawYinYangSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#2b2f3a', tintsToColor: true },
  { id: 'ferris_wheel', display: 'Ferris Wheel', painter: drawFerrisSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#d33', tintsToColor: true },
  { id: 'pinwheel', display: 'Pinwheel', painter: drawPinwheelSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#3fb6c8', tintsToColor: true },
  { id: 'watermelon', display: 'Watermelon', painter: drawWatermelonSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#e8453c', tintsToColor: true },
  { id: 'tire', display: 'Tire', painter: drawTireSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#c0392b', tintsToColor: true },
  { id: 'gear', display: 'Gear', painter: drawGearSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#9aa3ad', tintsToColor: true },
  { id: 'galaxy', display: 'Spiral Galaxy', painter: drawGalaxySkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#7a4fff', tintsToColor: true },
  { id: 'snowflake', display: 'Snowflake', painter: drawSnowflakeSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#5ad0ff', tintsToColor: true },
  { id: 'flower', display: 'Flower', painter: drawFlowerSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#ef6fb0', tintsToColor: true },
  { id: 'coin', display: 'Gold Coin', painter: drawCoinSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#3fb6c8', tintsToColor: true },
  { id: 'helm', display: 'Ship\'s Helm', painter: drawHelmSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#caa11a', tintsToColor: true },
  { id: 'aperture', display: 'Camera Aperture', painter: drawApertureSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#3f6fe0', tintsToColor: true },
  { id: 'cheese', display: 'Cheese Wheel', painter: drawCheeseSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#e8a13c', tintsToColor: true },
  { id: 'citrus', display: 'Citrus Slice', painter: drawCitrusSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#f2a72e', tintsToColor: true },
  { id: 'turtle', display: 'Turtle Shell', painter: drawTurtleSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#3f8f3a', tintsToColor: true },
  { id: 'pumpkin', display: 'Jack-o\'-Lantern', painter: drawPumpkinSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#e8731c', tintsToColor: true },
  { id: 'moon', display: 'Moon', painter: drawMoonSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#cfd2dc', tintsToColor: true },
  { id: 'ok_hand', display: 'OK Hand', painter: drawOkSkin, statue: true, tracksHeading: false, stateful: false, defaultColor: '#f5c518', tintsToColor: false },
  { id: 'mouse', display: 'Mouse', painter: drawMouseSkin, statue: false, tracksHeading: false, stateful: false, defaultColor: '#9aa3ad', tintsToColor: true },
];
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { FROZEN_CART_REGISTRY };
}
