// ============================================================================
//  chaochao — BORDER cosmetic painters (port-ready)
//  drawXxxBorder(ctx, anim, paint)
//
//  Normalized space: radius == 1 == kart rim, forward = +X. The integrator
//  (drawCartSkin in draw.js) does the translate/rotate/scale; painters draw the
//  border AROUND the rim (~1.0 .. ~1.4) and NEVER fill the interior (r < ~0.9) —
//  that's the cart skin's space.
//
//  PERF CONTRACT: no gradient/object allocation inside element loops; same-style
//  elements share ONE path + ONE fill/stroke; no per-element save/restore;
//  invariant shade strings hoisted above loops. Matches the flat-shade style of
//  drawFiretruckSkin / drawDinoSkin in draw.js. See bench results in
//  docs/spikes/ or the prototype's draw-loop meter.
//
//  Dual-environment: runs in the browser (uses the page's cartSkinShade if
//  present) and in Node (parses hex/rgb directly) so it can be unit-benched.
// ============================================================================
(function (global) {
  var TAU = Math.PI * 2;

  // --- colour helpers (mirror draw.js; Node-safe fallback) -------------------
  var _rgbCache = {};
  ...
  function parseColor(color) {
    if (typeof color !== "string") return { r: 200, g: 30, b: 30 };
  ...
  function cartSkinRGBA(color, a) {
    var c = cartSkinRGB(color);
    return "rgba(" + c.r + "," + c.g + "," + c.b + "," + a + ")";
  }

  // --- per-border params (the game wires its own; these are the defaults) -----
  var P = {
    ringThickness: 0.12, studCount: 12, dashCount: 10, glowRadius: 0.35,
    spikeLen: 0.34, spikeCount: 14, gearTeeth: 14, spinSpeed: 1.0
  };

  function drawRingBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var t = P.ringThickness, mid = 1.0 + t * 0.5;
  ...
  function drawStudsBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var r = 1.06, n = Math.max(4, Math.round(P.studCount));
  ...
  function drawDashedBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var r = 1.08, n = Math.max(3, Math.round(P.dashCount));
    var spin = anim * 0.6 * P.spinSpeed, arc = (TAU / n) * 0.55;
  ...
  function drawGlowBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var pulse = 0.5 + 0.5 * Math.sin(anim * 1.6 * P.spinSpeed);
    var outer = 1.0 + P.glowRadius * (0.7 + pulse * 0.5);
    var g = ctx.createRadialGradient(0, 0, 0.85, 0, 0, outer);
    g.addColorStop(0, cartSkinRGBA(paint, 0));
    g.addColorStop(0.55, cartSkinRGBA(paint, 0.32 + pulse * 0.25));
    g.addColorStop(1, cartSkinRGBA(paint, 0));
    ctx.beginPath(); ctx.arc(0, 0, outer, 0, TAU);
    ctx.fillStyle = g; ctx.fill();
    ctx.beginPath(); ctx.arc(0, 0, 0.99, 0, TAU);
    ctx.lineWidth = 0.05; ctx.strokeStyle = cartSkinRGBA(paint, 0.5 + pulse * 0.35); ctx.stroke();
  }

  function drawSpikesBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var n = Math.max(4, Math.round(P.spikeCount));
    var base = 1.0, tip = 1.0 + P.spikeLen, half = (Math.PI / n) * 0.6;
  ...
  function drawGearBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var n = Math.max(6, Math.round(P.gearTeeth));
    var spin = anim * 0.5 * P.spinSpeed;
  ...
  function drawElectricBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var bright = cartSkinShade(paint, 0.65), glow = cartSkinRGBA(paint, 0.35), ring = cartSkinRGBA(paint, 0.4);
    var arcs = 5, segs = 6;
    ...
    for (var k = 0; k < arcs; k++) {
      var phase = anim * (2.2 + k * 0.5) * P.spinSpeed + k * 2.399;
    ...
    for (var m = 0; m < arcs; m++) {
      var ph = anim * (2.2 + m * 0.5) * P.spinSpeed + m * 2.399;
  ...
  function drawChevronsBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var n = 12, r = 1.12, drift = anim * 0.5 * P.spinSpeed;
  ...
  function drawSawbladeBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var n = 18, spin = anim * 1.1 * P.spinSpeed, step = TAU / n, rIn = 1.0, rOut = 1.18;
  ...
  function drawFlamesBorder(ctx, anim, paint) {
    ...
    for (var i = 0; i < n; i++) {
      var a = (i / n) * TAU;
      var flick = 0.22 + 0.16 * (0.5 + 0.5 * Math.sin(anim * 3 * P.spinSpeed + i * 1.7));
    ...
    for (var j = 0; j < n; j++) {
      var a2 = (j / n) * TAU;
      var flick2 = 0.22 + 0.16 * (0.5 + 0.5 * Math.sin(anim * 3 * P.spinSpeed + j * 1.7));
  ...
  function drawRunesBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var pulse = 0.5 + 0.5 * Math.sin(anim * 1.5 * P.spinSpeed), bright = cartSkinShade(paint, 0.6);
    ctx.beginPath(); ctx.arc(0, 0, 1.08, 0, TAU);
    ctx.lineWidth = 0.02; ctx.strokeStyle = cartSkinRGBA(paint, 0.35); ctx.stroke();
    var n = 8, r = 1.08;
  ...
  function drawOrbitBorder(ctx, anim, paint) {
    paint = paint || "#cf3030";
    var track = 1.22, sats = 3, base = anim * 1.2 * P.spinSpeed;
    ctx.beginPath(); ctx.arc(0, 0, track, 0, TAU);
    ctx.lineWidth = 0.015; ctx.strokeStyle = cartSkinRGBA(paint, 0.3); ctx.stroke();
    ctx.beginPath();
    ...
    ctx.lineWidth = 0.06; ctx.lineCap = "round"; ctx.strokeStyle = cartSkinRGBA(paint, 0.35); ctx.stroke();
    ctx.lineCap = "butt";
    ctx.beginPath();
  ...
  function drawPlasmaBorder(ctx, anim, paint) {
    ...
    var rGlow = 1.0 + 0.12 + 0.05 * Math.sin(anim * 1.3 * P.spinSpeed);
    var g = ctx.createRadialGradient(0, 0, 0.9, 0, 0, rGlow + 0.18);
    g.addColorStop(0, cartSkinRGBA(paint, 0));
    g.addColorStop(0.6, cartSkinRGBA(paint, 0.3));
    g.addColorStop(1, cartSkinRGBA(paint, 0));
    edge(rGlow, 0.1, anim * 1.6 * P.spinSpeed);
    ctx.fillStyle = g; ctx.fill();
    edge(1.06, 0.07, anim * 2.0 * P.spinSpeed);
    ctx.lineWidth = 0.04; ctx.strokeStyle = cartSkinShade(paint, 0.55); ctx.stroke();
    edge(1.02, 0.05, -anim * 1.4 * P.spinSpeed);
    ctx.lineWidth = 0.025; ctx.strokeStyle = cartSkinRGBA(paint, 0.6); ctx.stroke();
  }

  var api = {
    P: P,
    cartSkinShade: cartSkinShade, cartSkinRGBA: cartSkinRGBA,
    drawRingBorder: drawRingBorder, drawStudsBorder: drawStudsBorder,
    drawDashedBorder: drawDashedBorder, drawGlowBorder: drawGlowBorder,
    drawSpikesBorder: drawSpikesBorder, drawGearBorder: drawGearBorder,
    drawElectricBorder: drawElectricBorder, drawLaurelBorder: drawLaurelBorder,
    drawChevronsBorder: drawChevronsBorder, drawDoubleBorder: drawDoubleBorder,
    drawTicksBorder: drawTicksBorder, drawScalesBorder: drawScalesBorder,
    drawSawbladeBorder: drawSawbladeBorder, drawFlamesBorder: drawFlamesBorder,
    drawRunesBorder: drawRunesBorder, drawOrbitBorder: drawOrbitBorder,
    drawCrownBorder: drawCrownBorder, drawPlasmaBorder: drawPlasmaBorder
  };
  if (typeof module !== "undefined" && module.exports) module.exports = api;
  else for (var key in api) global[key] = api[key];
