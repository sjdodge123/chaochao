...
- **Flame/delayed kills outside the attribution window aren't credited.** Kills are attributed via `murderedBy`, set from `punchedBy` at the moment of death (`player.js:656`) and consumed by `Game` to call `killer.addKill(...)` (`game.js:343-347`). But `setPunchedBy` clears `punchedBy` back to `null` after `c.playerKillWindow` ms (`player.js:~278`). When the kill is *delayed* — notably a flame/burn death that resolves a beat after the hit that doomed the victim — the window can expire before the victim actually dies, so `murderedBy` is `null` and **no one is credited the kill**. Fix: give delayed-damage sources (flame/burn) their own persistent attributor (e.g. an `ignitedBy`/`burnedBy` that lives as long as the burn) folded into `murderedBy` at death, or refresh/extend the kill window while a victim is still burning from your hit.
