        ...
        },
        {
            ...
            entries: [
                ...
                { id: "tile-bumper", name: "Bumpers", icon: art("bumper"), anim: "bumper",
                  ...
                  detail: "Bump one and you're flung away — maddening in a hurry, but a clever way to launch a chasing rival off their line. Some maps add moving bumpers that sweep across the track, so the safe lane keeps shifting, and some stretch the same springy sting into a bumper wall — a whole pinball-style line that kicks you straight off its face, great for shrugging karts back onto the racing line or guarding a shortcut. Other maps mount a bumper on a spinning arm — a rotor — that sweeps a full circle around its pivot like a clock hand, so the safe moment comes round and round. And watch the ground for geysers — vents that sit quiet, then bubble up a warning and erupt, launching anyone standing on or near them; the telegraph is your cue to step off. Proximity mines play the opposite game: they wait, armed and silent, until a kart strays too close, then blink down a fuse — a moment to scramble off if you're quick — before blowing up and flinging whoever's still nearby, so springing one as the leader can wreck the racers right behind you. A newer kind doesn't bonk you at all — it pulls you with a steady force. Vortex wells are the opposite of a bumper: they pull everything inside their ring toward the centre, so carry enough speed and you slingshot right past, but get caught crawling and you're reeled in — which is why you'll often find one parked over lava or just off the fast line. The very centre is calm, though: if you do get pulled in, keep driving and you'll build up speed in the quiet middle and power back out through the ring — annoying and slow, but never a dead end (unless the middle was sitting on lava). Two more play with timing rather than force. A laser gate is an energy barrier strung between two pylons that blinks on a steady cycle — open, then a shimmering warning, then solid — and while it's solid you can't pass: drive into it and you bounce straight off (it won't kill you, but a fence parked over lava turns that bounce into a problem), so read the rhythm and slip through on the gap. A crusher is a heavy slab that slides back and forth across a corridor on a rail, like a Thwomp: clip it mid-swing and it just shoves you aside, but get caught as it slams home against the wall and it crushes you flat — so time your run for when the slab's pulled back. And one hazard finally shoots back: a sentry turret is a fixed gun emplacement that tracks the nearest racer inside its firing arc, charges with a warning glow, then fires a glowing bolt at you. The bolt won't freeze or kill you — it bursts on impact and knocks you off your line — but a shove into lava or off a ledge does the rest, so it's nasty guarding a narrow pass. The charge-up is your tell: break out of the barrel's line while it's locking on and the shot fizzles, so juke across the arc rather than crawl through it. A wall or fence shields you completely — the turret can't see or shoot through a barrier, so put one between you and it. And it isn't invincible: a solid punch smashes it offline for the rest of the round, so if you can reach the emplacement you can take it out for good — easiest from outside its arc, where it can't shoot back. In an antlion round the turret also fires at the antlions, knocking them back when it connects. One hazard goes after your stash instead of your kart: a magpie drone patrols a rail and snatches the ability you're holding the moment it touches you, then flies off with it bobbing above its head — punch the drone and it drops the loot onto the ground as a pad anyone can grab, so a stolen Bomb or Star can change hands mid-race. Catch it empty-handed and there's nothing to take, so it just zaps a little of your stamina. A drone already carrying loot is harmless until someone frees it, so it's a steal-it, punch-it, grab-it-back scramble — and like a moving bumper it rides its rail, so you can time your dash through, but think twice if you're holding something good. Not every placeable is out to get you, though: some maps drop warp pads (a boon) — glowing portals that come in linked pairs. Drive onto one and you commit to a warp: the camera pulls back and sweeps across to the exit and you emerge at the partner pad, keeping all your speed and heading (you're invulnerable while you travel, so nobody can knock you out of the portal). The trip takes longer the farther apart the pads are, so a cross-map jump really reads as a journey. A well-placed pair is a real shortcut — but that travel time means it's only worth taking when it saves you a longer drive. The exit throws you out facing whatever way you came in, and an author can aim a partner right at lava — so punch a rival onto a pad whose far end opens over the fire and let the portal finish the job. Rival AI racers time the moving sweeps like you do — they hold at the edge of the bumper's path and dart through behind it, favoring the ends of its run where the gap stays open longest, they dash across a rotor's ring between passes, they steer wide of walls rather than ride them, they route around a vortex well's core, they wait for a laser gate to open or a crusher to pull back before crossing, they keep out of a sentry turret's line of fire, a bot carrying an ability gives a magpie drone a wide berth, and they'll happily route THROUGH a warp pad pair when it shortens their way to the goal rather than treating a pad as a dead end." },
