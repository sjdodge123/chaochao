...
// Author-placed barriers: solid fence/wall segments players can't cross (collision
// is server-side in engine.bounceOffBarriers). Drawn in the WORLD PASS (raw world
// coords, like hazards/bonus orbs) so the visual line matches the server collider.
// `style` only picks the look — every style is the same impassable segment. The art
// (drawBarrierFenceArt / drawBarrierConcreteArt) is shared VERBATIM with the editor
// preview + palette icons (create.js) — keep the two copies in sync.
function drawBarriers() {
    ...
    for (var i = 0; i < mapBarriers.length; i++) {
        var b = mapBarriers[i];
        if (b.style === "fence") { drawBarrierFenceArt(gameContext, b, 1); }
        else { drawBarrierConcreteArt(gameContext, b, 1); }
    }
}
